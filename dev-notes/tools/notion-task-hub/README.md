...
└── 任务总表（数据库）
    ...
    ├── 🎯 Focus 今日重点       ← 今日重点且未完成（完成后自动消失）
    ├── 🔲 四象限矩阵         ← 今日重点按重要/紧急四象限分列（完成后自动消失）
    ├── 📅 回溯视图         ← 日历，仅「已完成」
...
在任意视图把 **状态** 改为 **已完成**，任务会**立刻从 Focus 和四象限消失**（视图筛选：`今日重点 = 勾选 且 状态 ≠ 已完成`）。
...
可选：在「任务总表」⚡ 自动化里添加规则——状态变为「已完成」时自动取消 ☑️今日重点（并清空象限），避免「全部任务」里仍显示勾选。
...
2. 创建视图：
   ...
   - **Focus**：Layout 选 List，Filter `今日重点 = checked` **且** `状态 ≠ 已完成`
   - **四象限**：Layout 选 Board，Group by「象限」，Filter 同 Focus
