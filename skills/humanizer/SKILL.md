...
## Vault use: apply to every document output

In this vault, every document this agent produces or edits passes through this skill before you
hand it back: technical docs, design and architecture documents, reports, plans, READMEs, the
prose in `skills/*/SKILL.md`, commit and pull-request text, and Obsidian notes. Run the rewrite
inside the task that produced the text, then return only the final version, as in embedded mode.

Two outputs stay out of scope: Chat and CLI replies to the user, which keep the tool's speaking
style rather than a document voice, and the fenced blocks and inline code left untouched under
File mode.
